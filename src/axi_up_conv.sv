...
module axi_up_conv #(
  ...
  // memorize the previous upsizer selected for assignment
  tran_id_t                   idx_ar_upsizer_d, idx_ar_upsizer_q;
  ...
  // This is the ar_upsizer we selected when we accept this request
  assign idx_ar_upsizer_d = (arb_slv_ar_gnt) ? idx_idle_upsizer : idx_ar_upsizer_q;
  ...
  assign idx_r_upsizer = ar_fifo_out.upsizer_slot;
  ...
  r_state_e [AxiMaxReads-1:0] r_state_d, r_state_q;
  r_req_t   [AxiMaxReads-1:0] r_req_d  , r_req_q  ;
  ...
  always_comb begin
    ...
    if (mst_resp.r_valid && !ar_fifo_empty) begin
      ...
      slv_r_valid_tran[resp_upsizer] = 1'b1;

      // If the slave side is ready to accept the response, pop the FIFO and indicate master ready
      if (slv_r_ready_tran[resp_upsizer]) begin
        mst_r_ready_tran[resp_upsizer] = 1'b1;
        ar_fifo_pop = 1'b1; // pop the entry clearly
      end
    end
  end
  ...
  always_comb begin
    for (int t = 0; unsigned'(t) < AxiMaxReads; t++) begin: gen_read_upsizer
      // Are we idle?
      idle_read_upsizer[t] = (r_state_q[t] == R_IDLE) || (r_state_q[t] == R_INJECT_AW);

      // Maintain state
      r_state_d[t] = r_state_q[t];
      r_req_d[t]   = r_req_q[t]  ;
      ...
      // Throw an error
      mst_req_ar_err[t] = r_req_q[t].ar_throw_error;
      ...
      if (mst_ar_valid_tran[t] && mst_ar_ready_tran[t]) begin
        r_req_d[t].ar_valid       = 1'b0;
        r_req_d[t].ar_throw_error = 1'b0;
      end
      ...
      case (r_state_q[t])
        R_IDLE : begin
          ...
          if (arb_slv_ar_req && (idx_ar_upsizer == t)) begin
            ...
            if (inject_aw_into_ar) begin
              r_state_d[t] = R_INJECT_AW;
            end else begin
              // Default state
              r_state_d[t] = R_PASSTHROUGH;

              // Save beat
              r_req_d[t].ar           = slv_req_i.ar     ;
              r_req_d[t].ar_valid     = 1'b1             ;
              r_req_d[t].burst_len    = slv_req_i.ar.len ;
              r_req_d[t].orig_ar_size = slv_req_i.ar.size;

              case (r_req_d[t].ar.burst)
                axi_pkg::BURST_INCR: begin
                  // Modifiable transaction
                  if (modifiable(r_req_d[t].ar.cache)) begin
                    // No need to upsize single-beat transactions.
                    if (r_req_d[t].ar.len != '0) begin
                      // Evaluate output burst length
                      automatic addr_t start_addr = aligned_addr(r_req_d[t].ar.addr, AxiMstPortMaxSize);
                      automatic addr_t end_addr   = aligned_addr(beat_addr(r_req_d[t].ar.addr,
                          r_req_d[t].orig_ar_size, r_req_d[t].burst_len, r_req_d[t].ar.burst,
                          r_req_d[t].burst_len), AxiMstPortMaxSize);
                      r_req_d[t].ar.len  = (end_addr - start_addr) >> AxiMstPortMaxSize;
                      r_req_d[t].ar.size = AxiMstPortMaxSize                           ;
                      r_state_d[t]       = R_INCR_UPSIZE                               ;
                    end
                  end
                end
                ...
                axi_pkg::BURST_WRAP: begin
                  ...
                  r_state_d[t]              = R_PASSTHROUGH;
                  r_req_d[t].ar_throw_error = 1'b1         ;

                  // ... but might if this is a single-beat transaction
                  if (r_req_d[t].ar.len == '0)
                    r_req_d[t].ar_throw_error = 1'b0;
                end
              endcase
            end
        ...
        R_INJECT_AW : begin
          ...
          r_req_d[t].ar.id        = w_req_q.aw.id       ;
          r_req_d[t].ar.addr      = w_req_q.aw.addr     ;
          r_req_d[t].ar.size      = w_req_q.orig_aw_size;
          r_req_d[t].ar.burst     = w_req_q.aw.burst    ;
          r_req_d[t].ar.len       = w_req_q.burst_len   ;
          r_req_d[t].ar.lock      = w_req_q.aw.lock     ;
          r_req_d[t].ar.cache     = w_req_q.aw.cache    ;
          r_req_d[t].ar.prot      = w_req_q.aw.prot     ;
          r_req_d[t].ar.qos       = w_req_q.aw.qos      ;
          r_req_d[t].ar.region    = w_req_q.aw.region   ;
          r_req_d[t].ar.user      = w_req_q.aw.user     ;
          r_req_d[t].ar_valid     = 1'b0                ; // Injected "AR"s from AW are not valid.
          r_req_d[t].burst_len    = w_req_q.burst_len   ;
          r_req_d[t].orig_ar_size = w_req_q.orig_aw_size;
        ...
        R_PASSTHROUGH, R_INCR_UPSIZE: begin
          ...
          if (!r_req_q[t].ar_valid)
            if (mst_resp.r_valid && (idx_r_upsizer == t) && r_upsizer_valid) begin
              automatic addr_t mst_port_offset = AxiMstPortStrbWidth == 1 ? '0 : r_req_q[t].ar.addr[idx_width(AxiMstPortStrbWidth)-1:0];
              automatic addr_t slv_port_offset = AxiSlvPortStrbWidth == 1 ? '0 : r_req_q[t].ar.addr[idx_width(AxiSlvPortStrbWidth)-1:0];

              // Valid output
              // slv_r_valid_tran[t] = 1'b1                                       ;
              slv_r_tran[t].last  = mst_resp.r.last && (r_req_q[t].burst_len == 0);

              // Lane steering
              for (int b = 0; b < AxiMstPortStrbWidth; b++) begin
                if ((b >= mst_port_offset) &&
                    (b - mst_port_offset < (1 << r_req_q[t].orig_ar_size)) &&
                    (b + slv_port_offset - mst_port_offset < AxiSlvPortStrbWidth)) begin
                  r_data[t][b + slv_port_offset - mst_port_offset] = mst_resp.r.data[8*b +: 8];
                end
              end
              slv_r_tran[t].data = r_data[t];

              // Acknowledgment
              if (slv_r_ready_tran[t]) begin
                r_req_d[t].burst_len = r_req_q[t].burst_len - 1;

                case (r_req_q[t].ar.burst)
                  axi_pkg::BURST_INCR: begin
                    r_req_d[t].ar.addr = aligned_addr(r_req_q[t].ar.addr, r_req_q[t].orig_ar_size) + (1 << r_req_q[t].orig_ar_size);
                  end
                  axi_pkg::BURST_FIXED: begin
                    r_req_d[t].ar.addr = r_req_q[t].ar.addr;
                  end
                endcase
                ...
                //   R_INCR_UPSIZE:
                //     if (r_req_q[t].burst_len == 0 || (aligned_addr(r_req_d[t].ar.addr, AxiMstPortMaxSize) != aligned_addr(r_req_q[t].ar.addr, AxiMstPortMaxSize)))
                //       mst_r_ready_tran[t] = 1'b1;
                // endcase

                if (r_req_q[t].burst_len == '0) begin
                  r_state_d[t] = R_IDLE;
                  // ar_pop_per_upszier[t] = 1'b1;
                end
              end
            end
        end
  ...
  for (genvar t = 0; t < AxiMaxReads; t++) begin
    always_ff @(posedge clk_i or negedge rst_ni) begin
      if (!rst_ni) begin
        r_state_q[t] <= R_IDLE;
        r_req_q[t]   <= '0    ;
      end else begin
        r_state_q[t] <= r_state_d[t];
        r_req_q[t]   <= r_req_d[t]  ;
      end
    end
  end
